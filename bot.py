...
# ------------------------
# CONFIG (you can hardcode - but env-first is used)
# ------------------------
BOT_TOKEN = os.getenv("BOT_TOKEN") or "8119637737:AAEPDFrHrUeAHWHSHkR7ZSE_Hp1vg3sYs9c"
ADMIN_ID = int(os.getenv("ADMIN_ID") or 6812877108)   # your numeric telegram id
GROUP_ID = int(os.getenv("GROUP_ID") or -1003021667823)  # IVASMS forward group id
GROUP_LINK = os.getenv("GROUP_LINK") or "https://t.me/+NACzz1-K6dNiMjA9"
...
# IVASMS credentials (hardcode allowed by you)
...
# Scale & behavior
ASYNC_WORKERS = int(os.getenv("ASYNC_WORKERS") or 2000)
COOLDOWN_SECONDS = int(os.getenv("COOLDOWN_SECONDS") or 10)
MAX_NUMBERS_PER_USER = int(os.getenv("MAX_NUMBERS_PER_USER") or 10)
SCRAPE_INTERVAL = int(os.getenv("SCRAPE_INTERVAL") or 30)  # seconds
...
# ------------------------
# Scraping (blocking) - run with to_thread
# ------------------------
def login_and_scrape_numbers_sync():
    """
    Blocking function: logs into IVASMS and populates available_numbers table.
    ...
    try:
        ...
        if r.status_code != 200:
            print("[scrape.sync] login page status", r.status_code)
            return False
        soup = BeautifulSoup(r.text, "html.parser")
        t = soup.find("input", {"name": "_token"})
        if not t or not t.get("value"):
            print("[scrape.sync] No CSRF token on login page.")
            return False
        csrf = t.get("value")

        login_data = {"_token": csrf, "email": IVASMS_EMAIL, "password": IVASMS_PASSWORD}
        login_resp = scraper.post(LOGIN_URL, data=login_data, timeout=15, allow_redirects=False)
        ...
        if login_resp.status_code in (301,302) and "location" in login_resp.headers:
            ...
            if loc.startswith("/"):
                loc = BASE + loc
            portal_resp = scraper.get(loc, timeout=15)
        else:
            portal_resp = login_resp

        check_url = getattr(portal_resp, "url", "") or getattr(login_resp, "url", "")
        if "portal" not in check_url and "portal" not in portal_resp.text:
            print("[scrape.sync] Not in portal after login; login failed.")
            return False
        ...
        sms_resp = scraper.post(GET_SMS_URL, data=data, timeout=20)
        if sms_resp.status_code != 200:
            print("[scrape.sync] GET_SMS status", sms_resp.status_code)
            return False
        ...
        for rng in ranges:
            ...
            if r2.status_code != 200:
                print("[scrape.sync] GET_NUMBER failed for range", rng, "status", r2.status_code)
                continue
        ...
        conn.commit()
        conn.close()
        print("[scrape.sync] Numbers synced.")
        return True
    except Exception as e:
        print("[scrape.sync] Exception:", e)
        return False
...
dp = Dispatcher()
user_states = {}             # e.g. for withdraw / add user flows

# background scraping worker
async def background_scraper():
    while True:
        try:
            # run blocking scraping sync function in thread pool
            ok = await asyncio.to_thread(login_and_scrape_numbers_sync)
            if not ok:
                print("[bg] scrape failed")
        except Exception as e:
            print("[bg] exception", e)
        await asyncio.sleep(SCRAPE_INTERVAL)

# handle forwarded messages from IVASMS group
...
# Admin text flows (add number / add user)
@dp.message()
...
@dp.message()
async def generic_message(m: types.Message):
    # prevent falling through; do nothing
    return
